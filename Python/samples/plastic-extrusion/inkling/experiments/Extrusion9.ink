...
function MaskFunction(State: SimState) {
    if State.product_length > State.target_length {
        return constraint SimAction {
            screw_angular_acceleration: number <-ScrewAccelerationMax .. 0>,
            cutter_acceleration: number <0 .. CutterAccelerationMax>
        }
    } else if State.product_length < State.target_length {
        return constraint SimAction {
            screw_angular_acceleration: number <0 .. ScrewAccelerationMax>,
            cutter_acceleration: number <-CutterAccelerationMax .. 0>
        }
    } else {
        return constraint SimAction {}
    }
...
graph (input: SimState): SimAction {
    ...
    concept DriveLength(input): SimAction {
        curriculum {
