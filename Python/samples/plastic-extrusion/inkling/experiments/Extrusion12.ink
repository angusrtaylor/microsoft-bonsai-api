...
const LengthTarget3 = 1.5 * LengthMultiplier
...
type SimConfig {
    ...
    target_length: number <0.5 * LengthMultiplier, 1 * LengthMultiplier, 1.5 * LengthMultiplier>,
...
# function disallowMaxYield(State: SimState):number {
#     if State.product_length > State.target_length + LengthTolerance {
#         return 1 # disallow
#     } else if State.product_length < State.target_length - LengthTolerance {
#         return 1 # disallow
#     } else {
#         return 0 # allow
#     }
...
function disallowMaxYieldDriveLength1(State: SimState):number {
...
function disallowMaxYieldDriveLength2(State: SimState):number {
    if State.target_length != LengthTarget2 {
        return 1 # disallow
    } else {
        return 0
    }
...
graph (input: SimState): SimAction {
    ...
    concept MaxYieldDriveLength1(input): SimAction {
        curriculum {
    ...
    concept MaxYieldDriveLength2(input): SimAction {
        curriculum {
    ...
    concept MaxYieldDriveLength3(input): SimAction {
        curriculum {
    ...
    output concept Selector(input): SimAction {
        select MaxYieldDriveLength1 {
            mask disallowMaxYieldDriveLength1
        }
        select MaxYieldDriveLength2 {
            mask disallowMaxYieldDriveLength2
        }
        select MaxYieldDriveLength3
        curriculum {
            ...
            lesson RandomizeStartWide {
                ...
                scenario {
                    ...
                    initial_cutter_acceleration: number <-CutterAccelerationMax .. CutterAccelerationMax>,                
                    initial_temperature: InitialTemperature,
                    target_length: number <LengthTarget1, LengthTarget2, LengthTarget3>
                }
            }
        }
    }
